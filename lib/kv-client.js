...
export class QuizStorage {
  ...
  async updateQuestionProgress(questionId, isCorrect, metadata = {}) {
    ...
    try {
      ...
      const isFirstAttempt = (current.apariciones || 0) === 0;
      ...
      if (metadata.category) {
        current.categoria = metadata.category;
      }
      if (metadata.lastAnswer != null) {
        current.ultima_respuesta = metadata.lastAnswer;
      }
      ...
      if (isFirstAttempt) {
        await kv.hincrby('user:stats', 'total_questions_seen', 1);
      }
      ...
      return {
        ...
        ultima_respuesta: metadata.lastAnswer,
  ...
  async getUserStats() {
    try {
      ...
      return {
        total_tests: parseInt(stats.total_tests || 0, 10) || 0,
        average_score: parseFloat(stats.average_score || 0) || 0,
        last_test: stats.last_test || null,
        total_questions_seen: parseInt(stats.total_questions_seen || 0, 10) || 0
      };
    } catch (error) {
  ...
  async getUserConfig() {
    try {
      ...
      return {
        ...
        timer_minutes: parseInt(config.timer_minutes || 120, 10) || 120,
        show_feedback: config.show_feedback !== 'false',
        show_statistics: config.show_statistics !== 'false',
        questions_per_test: parseInt(config.questions_per_test || 100, 10) || 100
      };
    } catch (error) {
